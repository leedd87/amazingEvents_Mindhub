...
async function getDataEventos() {
   ...
   let maximoPorcentajeYEvento = arrayPorcentajeYEvento.sort((a, b) => b.porcentajeAssistance - a.porcentajeAssistance)[0];

   //console.log(maximoPorcentajeYEvento)

   let minimoPorcentajeYEvento = arrayPorcentajeYEvento.sort((a, b) => a.porcentajeAssistance - b.porcentajeAssistance)[0];
   ...
   /*TABLA 2*/
   let arrayCategoriasFuturas = arrayEventosFuturos.map(element => element.category)
   ...
   arrayCategoriasFuturas.map(categoria => {
      categoriaValueFuturo.push({
         category: categoria,
         evento: arrayEventosFuturos.filter(evento => evento.category === categoria)
      })
   })
   ...
   // /*TABLA 3 */
   let arrayCategoriasPasadas = arrayEventosPasados.map(element => element.category)
   ...
   arrayCategoriasPasadas.map(categoria => {
      ...
      categoriaValuePasado.push({
         category: categoria,
         evento: arrayEventosPasados.filter(evento => evento.category === categoria)
